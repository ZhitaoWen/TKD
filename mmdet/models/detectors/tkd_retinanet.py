...
class TKDRetinaNet(CrossKDSingleStageDetector):
    ...
    def forward_crosskd_single(self, x, module):
        ...
        for i, cls_conv in enumerate(module.bbox_head.cls_convs):
            cls_feat = cls_conv(cls_feat, activate=False) # no relu feat as  cls_feat_hold
            if i + 1 == self.reused_teacher_head_idx:
                cls_feat_hold = cls_feat
            cls_feat = cls_conv.activate(cls_feat) # add relu
