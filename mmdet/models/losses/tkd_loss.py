...
class TKDLoss(nn.Module):
    ...
    def __init__(self,
        ...
        self.ins_rea = ins_rea
        self.pixel_rea = pixel_rea
        self.sed = sed
    ...
    def forward(self,
        ...
        ins_loss = self.get_ins_rela_loss(ins_feats_T, ins_feats_S, N)
        pixel_loss = self.get_pixel_rela_loss(preds_S, preds_T)

        sed_loss = self.get_sed_loss(preds_S, preds_T)

        loss = self.alpha_fgd * fg_loss + self.beta_fgd * bg_loss \
               + self.gamma_fgd * mask_loss + self.ins_rea * ins_loss + self.pixel_rea * pixel_loss + self.sed * sed_loss
    ...
    def get_ins_rela_loss(self, roi_T, roi_S, N):
        B, C, H, W = roi_T.shape
